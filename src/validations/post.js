...
const postStatuses = ["sent", "not-send"];
...
export const validations = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return res
        .status(422)
        .json(errorResponse(req.__("Invalid or past scheduleDate")));
    }
    ...
    // Combine 'scheduleDate' and 'scheduleTime' to create a complete datetime
    const scheduleDateTimeCombined = new Date(
      `${req.body.scheduleDate}T${req.body.scheduleTime}:00`
    );
    if (scheduleDateTimeCombined < currentDateTime) {
      return res
        .status(422)
        .json(
          errorResponse(
            req.__("scheduleDate and scheduleTime must be in the future")
          )
        );
    }
    ...
    if (
      ...
      return res
        .status(422)
        .json(errorResponse(req.__("Invalid or past scheduleDateTime")));
    }
